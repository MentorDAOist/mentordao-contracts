...
import {IMentorNFTBase} from "../../interfaces/IMentorNFTBase.sol";
 ...
 * @author MentorDAO
 *
 * @notice This is an abstract base contract to be inherited by other MentorDAO NFTs.
 */
abstract contract MentorNFTBase is IMentorNFTBase {
    ...
    /// @inheritdoc IMentorNFTBase
    function permitForAll(address owner, address operator, bool approved, DataTypes.EIP712Signature calldata sig)
        external
        override
    {
    ...
    function _calculateDomainSeparator() internal view returns (bytes32) {
        return keccak256(
            abi.encode(
                EIP712_DOMAIN_TYPEHASH, keccak256(bytes(name())), EIP712_REVISION_HASH, block.chainid, address(this)
            )
        );
    }
